...
# convert y_train from [60000,] to [60000, 11]
...
H = Convolution2D(32, (5, 5), strides=(1, 1), padding = 'same', activation='relu')(d_input)
H = Convolution2D(32, (5, 5), strides=(2, 2), padding = 'same', activation='relu')(H)
H = LeakyReLU(0.2)(H)
H = Dropout(dropout_rate)(H)
H = Convolution2D(64, (5, 5), strides=(1, 1), padding = 'same', activation='relu')(H)
H = Convolution2D(64, (5, 5), strides=(2, 2), padding = 'same', activation='relu')(H)
H = LeakyReLU(0.2)(H)
H = Dropout(dropout_rate)(H)
H = Convolution2D(128, (5, 5), strides=(1, 1), padding = 'same', activation='relu')(H)
...
if commandline_args.load:
    generator.load_weights("cifar_ss_generator.h5")
    discriminator.load_weights("cifar_ss_discriminator.h5")
...
def train_for_n(epochs=5000, plt_frq=25,BATCH_SIZE=32):
    ...
    for e in tqdm(range(epochs)):  
        ...
        image_batch = X_train[sample_indices,:,:,:]
        label_batch = y_train[sample_indices,:]
        ...
        y[0:BATCH_SIZE,:] = label_batch[0:BATCH_SIZE,:]
        y[BATCH_SIZE:,10] = 1
        ...
        if e%plt_frq==plt_frq-1:
            generator.save_weights("cifar_ss_generator.h5")
            discriminator.save_weights("cifar_ss_discriminator.h5")
